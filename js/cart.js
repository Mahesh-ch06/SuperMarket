...
function removeItem(productId) {
    openConfirmationModal('Are you sure you want to remove this item?', () => {
        ...
        window.FreshMartCommon.showToast('Item removed from cart!', 'success');
    });
}
...
function emptyCart() {
    openConfirmationModal('Are you sure you want to empty your cart?', () => {
        ...
        renderCart();
        closeConfirmationModal();
        window.FreshMartCommon.showToast('Cart emptied successfully!', 'success');
    });
}
...
async function proceedToCheckout() {
    ...
    if (cartItems.length === 0) {
        ...
        window.FreshMartCommon.showToast('Your cart is empty. Please add items before checking out.', 'info');
        return;
    }
    ...
    openConfirmationModal('Proceed to checkout with your current items?', async () => {
        ...
        try {
            ...
            // Create order data for localStorage
            const orderData = {
            ...
            console.log('✅ Order placed successfully with ID:', orderData.id);
            
            window.FreshMartCommon.showToast('🎉 Order placed successfully!', 'success', 4000);
            window.FreshMartCommon.showToast('📧 Order confirmation sent to your email!', 'info', 3000);
            ...
            setTimeout(() => {
                window.FreshMartCommon.showToast('Redirecting to My Orders...', 'info', 2000);
                setTimeout(() => {
            ...
            window.FreshMartCommon.showToast(`❌ Failed to place order: ${error.message}`, 'error', 6000);
        } finally {
